...
router.delete("/account", requireAuth, async (req, res) => {
    try {
        await prisma.$transaction(
            async (tx) => {
                // Last-admin guard INSIDE the txn so count-then-delete is atomic under
                // Serializable isolation: two concurrent admin deletions can't both see
                // count>1 and leave the instance with zero admins (a bricked instance).
                ...
                // UserMoodMix and DiscoveryBatch carry a bare `userId` with no @relation
                // / onDelete: Cascade — delete them manually before the user row (there's
                // no FK to cascade or to violate, so otherwise they orphan).
                await tx.userMoodMix.deleteMany({ where: { userId: req.user!.id } });
                await tx.discoveryBatch.deleteMany({ where: { userId: req.user!.id } });
        ...
        // Serializable conflict from a concurrent admin change — safe to retry.
        if (error?.code === "P2034") {
            return res.status(409).json({
                error: "Concurrent account change, please retry",
            });
        }
